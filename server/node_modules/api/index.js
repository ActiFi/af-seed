...
var exportApi = {
  ...
  loadRoute:function(route, router){
    ...
    if(!_.isPlainObject(route))
       bomb.boom('Route definitions must be an object. Received:' + typeof route, 'Invalid Route').explode();
    if(!_.has(route, 'path'))
       bomb.boom('Routes require a path property.').explode();
    if(!_.has(route, 'handler'))
       bomb.boom('Routes require a handler property. Invalid Route: ' + route.path).explode();
    ...
    if(route.access && route.access.length){
      _.each(route.access, function(accessName){
        if(_.has(exportApi.schemas, accessName))
          ...
          bomb.boom(accessName + ' security schema not found for route ' + route.path).explode();
      });
    }
